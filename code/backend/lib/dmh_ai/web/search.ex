...
defmodule DmhAi.Web.Search do
  ...
  Exposes:
    plan_turn/3                — one swift LLM call per user message: web-search decision +
                                 ...
                                 Returns `%{search:, memory_query:, deliverable:}`. The
  ...
  Returns `%{search: {:no_search} | {:search, category, queries},
             memory_query: String.t(), deliverable: boolean()}`. On LLM error
  it degrades to no-search / no memory query / not-deliverable.
  """
