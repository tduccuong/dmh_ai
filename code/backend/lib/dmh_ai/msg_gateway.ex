...
defmodule DmhAi.MsgGateway do
  ...
  In config/config.exs (or runtime.exs):
      ...
      config :dmh_ai, :msg_platforms, [:telegram]   # default

  Adding a platform
  -----------------
  1. Create `lib/dmh_ai/adapters/<platform>.ex` implementing DmhAi.Agent.Adapter.
  ...
  defp platforms do
    Application.get_env(:dmh_ai, :msg_platforms, @default_platforms)
  end
end
