...
defmodule DmhAi.Tools.ExtractContent do
  ...
  defp scale_and_encode(path) do
    tmp = "/tmp/dmh_ai_ec_#{System.unique_integer([:positive])}.jpg"
  ...
  defp extract_frames(video_path) do
    dir = "/tmp/dmh_ai_ec_frames_#{System.unique_integer([:positive])}"
    File.mkdir_p!(dir)
  ...
  defp render_and_ocr(path, n_pages, ctx) do
    dir = "/tmp/dmh_ai_ocr_#{System.unique_integer([:positive])}"
    File.mkdir_p!(dir)
    rid = row_id(ctx)
