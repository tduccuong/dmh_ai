...
defmodule DmhAi.Tools.AuthorizeService do
  ...
  runtime matches the target against catalog entries and:
    ...
    * If the target doesn't match any catalog entry → return a
      ...
      (web_search for public info, or honest decline).
  ...
  # and offer fallbacks (web_search, honest decline).
  defp none_message(_target_in, []) do
    "No OAuth services are configured in this deployment. Tell the user honestly: this DMH-AI install has no OAuth catalog yet (the operator hasn't wired any). Offer alternatives: `web_search` (for public info that doesn't need auth) or honest decline. Suggest they ask the operator to add the service to the OAuth catalog."
  end
  ...
  defp none_message(target_in, candidates) do
    "No service in the OAuth catalog matches `#{target_in}`. Configured services nearest to your input:\n" <>
      ...
      "If none of the candidates fit and the user can't supply a URL, the service isn't wired up here — offer " <>
      "`web_search` (for public info) or honest decline. Don't guess and retry."
  end
