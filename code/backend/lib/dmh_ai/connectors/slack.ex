...
defmodule DmhAi.Connectors.Slack do
  ...
  Eight functions at the SME-relevant slice of Slack's Web API
  (api.slack.com/methods) — messages, channels, search, users,
  reactions:
  ...
  Four capability groups (messaging / channels / search / directory)
  so admins can scope per-org — a notify-only org might tick
  messaging, while a support org also enables search + directory.
  ...
  def remap_error(%{"ok" => false, "error" => code}) when is_binary(code) do
    cond do
      code in ["already_reacted", "name_taken"] ->
        :duplicate
  ...
  — so a notify-only org can expose messaging and skip the rest,
  while a support org also enables search + directory. The three
  ...
  def capabilities do
    [
      %{
        ...
        description:  "Post and edit messages; add emoji reactions.",
        scopes:       ["chat:write", "reactions:write"],
        functions:    ["message.send", "message.update", "reaction.add"],
        vendor_prereq: %{
      ...
      %{
        ...
        description:  "List channels and read channel history.",
        scopes:       ["channels:read", "channels:history"],
        functions:    ["channel.find", "channel.history"],
        vendor_prereq: %{
      ...
      %{
        ...
        description:  "Look up workspace users by email or list them.",
        scopes:       ["users:read", "users:read.email"],
        functions:    ["user.find_by_email", "user.list"],
        vendor_prereq: %{
