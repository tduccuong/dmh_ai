...
# Flow F36 — HubSpot read verb e2e via POST /tools/execute (Phase C slice 1).
#
# Full HTTP path:
#   POST /tools/execute (admin token)
#     → Router → Handlers.Tools.post_execute
#     → Tools.Registry.execute("hubspot.deal.find", args, ctx)
#     → Dispatcher.call/3 (gates pass: read, callable_from chat+task)
#     → MCPAdapter.HubSpot.call/3
#     → Caller.invoke (stubbed) returns fake deal
#     → response surfaces in the HTTP body as ok: true, result: {...}
#
# Verifies the HARD contract: a read verb works from free chat (no
# task_id required) and the dispatcher does NOT inject an
# idempotency_key on reads. No audit_log denial.

defmodule DmhAi.Flows.F36HubspotReadFreeChat do
  ...
  @moduletag flow_id: "F36"

  setup_all do
    teardown = DmhAi.Test.FlowHelper.setup_profile("F36")
    Dispatcher.reset()
  ...
  test "POST /tools/execute hubspot.deal.find (read, free chat) → 200 with deal",
       %{admin_id: admin_id, email: email, password: password} do
    Application.put_env(:dmh_ai, :__mcp_caller_stub__, fn "hubspot", "deal.find", args, _creds ->
    ...
    assert get_in(decoded, ["result", "deals"]) |> is_list()
    assert hd(decoded["result"]["deals"])["id"] == "d-001"
