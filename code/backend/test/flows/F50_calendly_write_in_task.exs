...
# Flow F50 — Calendly write function inside an active task succeeds.
# Mirrors F38 (HubSpot write in task). Verifies the idempotency_key
# derivation contract holds for an in-process Case-A connector.

defmodule DmhAi.Flows.F50CalendlyWriteInTask do
  ...
  @moduletag flow_id: "F50"

  setup_all do
    teardown = DmhAi.Test.FlowHelper.setup_profile("F50")
    Dispatcher.reset()
  ...
  test "POST /tools/execute calendly.single_use_link.create with task_id → success, idempotency_key, audit row",
       %{admin_id: admin_id, email: email, password: password} do
    keys_table = :ets.new(:f50_keys, [:public, :set])
    ...
    Application.put_env(:dmh_ai, :__mcp_caller_stub__, fn "calendly", "single_use_link.create", args, _creds ->
      ...
      {:ok, %{"booking_url" => "https://calendly.com/d/abc-42"}}
    end)
    ...
    assert decoded["result"]["booking_url"] == "https://calendly.com/d/abc-42"

    # Idempotency key derivation contract.
    [{_, captured_key}] = :ets.tab2list(keys_table)
    expected_key =
      :crypto.hash(:sha256, "task-f50\0#{tool_call_id}\0calendly.single_use_link.create")
