...
# Flow F42 — Google Workspace read verb e2e via POST /tools/execute.
# Mirrors F36/F39 for the GW connector.

defmodule DmhAi.Flows.F42GWorkspaceReadFreeChat do
  ...
  @moduletag flow_id: "F42"

  setup_all do
    teardown = DmhAi.Test.FlowHelper.setup_profile("F42")
    Dispatcher.reset()
  ...
  test "POST /tools/execute google_workspace.gmail.search (read, free chat) → 200",
    ...
    Application.put_env(:dmh_ai, :__mcp_caller_stub__, fn "google_workspace", "gmail.search", args, _ ->
      ...
      assert args["query"] == "label:inbox"
      {:ok, %{"messages" => [%{"id" => "g-001", "subject" => "Sprint review"}]}}
    end)

    token = login_for_token(email, password)

    conn = post_json("/tools/execute",
                     %{"name" => "google_workspace.gmail.search",
                       "args" => %{"query" => "label:inbox"}},
                     token: token)
    ...
    assert hd(decoded["result"]["messages"])["id"] == "g-001"
