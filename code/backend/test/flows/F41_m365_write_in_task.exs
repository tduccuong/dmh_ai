...
# Flow F41 — M365 write verb succeeds inside an active task.
#
# Mirrors F38. POST /tools/execute m365.mail.send with task_id set
# in the body. Asserts:
#
#   * HTTP 200 with `message_id` returned.
#   * MCP Caller received an `__idempotency_key` derived from
#     (task_id, tool_call_id, verb) per the dispatcher contract.
#   * audit_log row written with action='write', outcome='allowed'.
#   * Two requests with the same task_id + tool_call_id produce the
#     SAME idempotency_key → Graph's send-mail dedup fires on retry.

defmodule DmhAi.Flows.F41M365WriteInTask do
  ...
  @moduletag flow_id: "F41"

  setup_all do
    teardown = DmhAi.Test.FlowHelper.setup_profile("F41")
    Dispatcher.reset()
  ...
  test "POST /tools/execute m365.mail.send with task_id → success, idempotency_key, audit row",
    ...
    assert decoded["result"]["message_id"] == "msg-42"

    [{_, captured_key}] = :ets.tab2list(keys_table)
    expected_key =
      :crypto.hash(:sha256, "task-f41\0#{tool_call_id}\0m365.mail.send")
