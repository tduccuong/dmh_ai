...
# Flow F44 — GW write verb succeeds inside an active task. Mirrors F38/F41.

defmodule DmhAi.Flows.F44GWorkspaceWriteInTask do
  ...
  @moduletag flow_id: "F44"

  setup_all do
    teardown = DmhAi.Test.FlowHelper.setup_profile("F44")
    Dispatcher.reset()
  ...
  test "POST /tools/execute google_workspace.gmail.send with task_id → success + idempotency_key",
       %{admin_id: admin_id, email: email, password: password} do
    keys_table = :ets.new(:f44_keys, [:public, :set])
    ...
    assert decoded["result"]["message_id"] == "gmsg-42"

    [{_, captured_key}] = :ets.tab2list(keys_table)
    expected_key =
      :crypto.hash(:sha256, "task-f44\0#{tool_call_id}\0google_workspace.gmail.send")
